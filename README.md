...
The whole Express app runs as a single Vercel serverless function (the
documented Express-on-Vercel pattern), which also serves the static assets.

How it is wired:

- **`api/index.ts`** exports the Express `app` as the serverless handler.
- **`vercel.json`** (`@vercel/node`) bundles `views/` and `public/` into the
  function (`includeFiles`) and routes every request to it.
- **`app.ts`** detects Vercel (`process.env.VERCEL`) to enable `trust proxy`
  (correct client IP for rate-limiting) and skip `app.listen()`, and resolves
  the `views/`+`public/` dirs robustly inside the bundle.
- The built **`public/css/styles.css` is committed** so no build step runs on
  Vercel. After editing styles, run `npm run build:css` and commit the result.

Steps:

1. Push this repo to GitHub and "Import Project" in Vercel (no framework preset).
   Or run `npx vercel` from the repo.
...
- `robots.txt`, `sitemap.xml` and `/favicon.ico` are generated by the app;
  everything in `public/` (CSS, JS, logos, images, favicons, manifest) is served
  by the function with cache headers.
